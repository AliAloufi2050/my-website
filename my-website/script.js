...
let siteData = {
    ...
    contact: { title: "📞 تواصل معي", email: "amaaz@example.com", twitter: "@AMAz", whatsapp: "05XXXXXXXX", message: "يمكنك التواصل معي عبر القنوات التالية" },
    media: { images: [] },
    incoming: { 
        messages: [
            { id: 1, userId: null, name: "أحمد", email: "ahmed@example.com", message: "موقع رائع!", date: new Date().toISOString(), status: "unread" }
        ] 
    },
...
function loadPage(page) {
    ...
    if (page === 'home') {
        ...
        html = `<h1>${siteData.contact.title}</h1><p>${siteData.contact.message}</p><ul><li>📧 ${siteData.contact.email}</li><li>🐦 ${siteData.contact.twitter}</li><li>📱 ${siteData.contact.whatsapp}</li></ul>`;
    } else if (page === 'admin') {
...
// ========================================
// إدارة الوارد (كل حساب له رسائله الخاصة)
// ========================================
function showCommentsAndIncoming() {
    ...
    // تصفية الرسائل حسب المستخدم الحالي (المالك يشوف الكل)
...
function setupPremiumChatbot() {
    ...
    // قاعدة المعرفة المتقدمة للشات بوت
    const knowledge = {
        greetings: ['مرحب', 'هلا', 'سلام', 'اهلا', 'hello', 'hi', 'مرحبا', 'السلام عليكم'],
        about: ['من انت', 'وش اسمك', 'who are you', 'bot', 'روبوت', 'اسمك'],
        programming: ['برمجة', 'لغة برمجة', 'بايثون', 'جافا', 'javascript', 'html', 'css', 'كود', 'تعلم برمجة'],
        web: ['موقع', 'تصميم موقع', 'كيف اسوي موقع', 'استضافة', 'تطوير ويب'],
        ai: ['ذكاء اصطناعي', 'ai', 'chatgpt', 'gemini', 'تعلم آلة'],
        stats: ['احصائيات', 'زيارات', 'stats', 'إحصائيات', 'أرقام', 'عدد'],
        theme: ['داكن', 'فاتح', 'ثيم', 'مظهر', 'لون', 'الوضع', 'dark', 'light'],
        projects: ['مشاريع', 'project', 'اعمال', 'portfolio'],
        contact: ['تواصل', 'اتصال', 'بريد', 'email', 'واتساب', 'جوال', 'كيف اتواصل'],
        help: ['مساعدة', 'help', 'كيف', 'شرح', 'طريقة', 'وش تسوي'],
        thanks: ['شكرا', 'thank', 'thanks', 'تسلم', 'ممتاز', 'جميل', 'رائع'],
        achievements: ['انجازات', 'إنجازات', 'achievements', 'جوائز'],
        blog: ['مدونة', 'blog', 'مقالات', 'تدوينات'],
        time: ['الوقت', 'الساعة', 'كم الساعة', 'time', 'تاريخ اليوم']
    };
    
    const responses = {
        greetings: ["✨ أهلاً وسهلاً! كيف أقدر أساعدك اليوم؟", "🤖 مرحباً بك في AMA'z! أنا مساعدك الذكي.", "💫 يا هلا بك! تفضل اسأل وأنا أجاوبك على طول."],
        about: ["🤖 أنا AMAz Bot، مساعد ذكي متطور مثل Google Gemini! اسألني أي شيء.", "✨ أنا روبوت محادثة ذكي، مهمتي مساعدتك في استخدام منصة AMA'z."],
        programming: ["💻 أفضل لغة للمبتدئين هي Python. تريد شرح مفصل؟", "📚 ابدأ بـ HTML/CSS ثم JavaScript، وبعدها اختر مسارك المفضل."],
        web: ["🌐 لتصميم موقع: HTML للهيكل، CSS للتجميل، JavaScript للتفاعل. ترفعه مجاناً على Netlify."],
        ai: ["🧠 الذكاء الاصطناعي يحاكي العقل البشري. أشهر النماذج: ChatGPT و Gemini."],
        stats: [`📊 إجمالي الزيارات: ${siteData?.stats?.totalVisits || 0} | اليوم: ${siteData?.stats?.todayVisits || 0} | الإنجازات: ${siteData?.achievements?.list?.length || 0}`],
        theme: ["🎨 اضغط على زر 🌙/☀️ في أعلى الشاشة. الوضع الحالي: " + (document.body.classList.contains('dark-mode') ? 'داكن 🌙' : 'فاتح ☀️')],
        projects: [`📁 عدد المشاريع: ${siteData?.projects?.list?.length || 0}. شاهدها من القائمة العلوية.`],
        contact: [`📞 البريد: ${siteData?.contact?.email} | تويتر: ${siteData?.contact?.twitter} | واتساب: ${siteData?.contact?.whatsapp}`],
        help: ["🆘 اسألني عن: البرمجة، تصميم المواقع، إحصائيات الموقع، أو أي شيء تقني."],
        thanks: ["🙏 العفو! أنا هنا لخدمتك دائماً 🤍", "💙 شكراً لك! تشرفت بمساعدتك."],
        achievements: [`🏆 عدد الإنجازات: ${siteData?.achievements?.list?.length || 0}. شاهدها من القائمة العلوية.`],
        blog: [`✍️ عدد التدوينات: ${siteData?.blog?.posts?.length || 0}. أقرأها من المدونة.`],
        time: [`🕐 الوقت: ${new Date().toLocaleTimeString('ar-SA')} | 📅 التاريخ: ${new Date().toLocaleDateString('ar-SA')}`]
    };
    
    function getResponse(msg) {
        const input = msg.toLowerCase().trim();
        for (let category in knowledge) {
            for (let keyword of knowledge[category]) {
                if (input.includes(keyword)) {
                    const catResponses = responses[category];
                    if (catResponses) {
        ...
        return "🤔 سؤال عميق! هل تقصد في مجال البرمجة والتقنية؟ أنا متخصص في هذا المجال. جرب تسأل عن البرمجة، تصميم المواقع، أو إحصائيات الموقع.";
    }
...
const playlist = [
    ...
    { id: '7NOSDKb0HlU', title: 'Jazz Vibes', artist: 'Coffee Shop' }
];
